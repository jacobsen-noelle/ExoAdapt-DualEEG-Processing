...
% %Noelle Jacoben Modfied 9/9/21
%   %%Made code compatible with colors in [R G B] format
...
function plotcurve_colors( times, R, varargin);
  ...
  for ind = 1:size(R,1)
      ...
      elseif ~isempty(g.colors),
           ...
           if length(tmpcol) > 1 %%NJ, added check for colors in [R G B] format
               if ~strcmp(class(tmpcol),'double')
               tmpstyle = tmpcol(2:end); tmpcol = tmpcol(1);
               else
                   tmpstyle = '-';
               end
           else
           ...
           set(tmp, 'color', tmpcol, 'linestyle', tmpstyle,'linewidth',2); %%NJ changed line width 
